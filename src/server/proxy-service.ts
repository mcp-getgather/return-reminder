...
export class ProxyService {
  ...
  async getClientLocation(ipAddress: string): Promise<City | null> {
    Logger.debug('[ProxyService] Getting client location', { ipAddress });
  ...
  async reverseProxy(
    ...
    Logger.debug('[ProxyService] Client location data', {
      city: clientLocation?.city,
      country: clientLocation?.country,
      state: clientLocation?.state,
      postalCode: clientLocation?.postal_code,
    });
    ...
    if (contentType.toLowerCase().includes('application/json')) {
      try {
        ...
        Logger.error(
          '[ProxyService] Error processing JSON body',
          error as Error
        );
      }
    }
    ...
    try {
      ...
      Logger.debug('[ProxyService] Proxy request', { targetUrl });
      ...
      // Send response
      Logger.debug('[ProxyService] Upstream response', {
